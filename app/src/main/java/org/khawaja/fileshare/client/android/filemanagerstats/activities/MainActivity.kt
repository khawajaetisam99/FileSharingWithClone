...
import org.khawaja.fileshare.client.android.filemanagerstats.adapters.ViewPagerAdapter
...
import org.khawaja.fileshare.client.android.filemanagerstats.fragments.ItemsFragment
...
import org.khawaja.fileshare.client.android.filemanagerstats.helpers.MAX_COLUMN_COUNT
import org.khawaja.fileshare.client.android.filemanagerstats.helpers.RootHelpers
import org.khawaja.fileshare.client.android.filemanagerstats.helpers.tabsList
...
import org.khawaja.fileshare.client.android.databinding.ActivityMainBinding
import java.io.File
import java.util.*
import androidx.fragment.app.FragmentTransaction
import org.khawaja.fileshare.client.android.fragment.HomeListContainer
...
class MainActivity : SimpleActivity() {
    ...
    override fun onResume() {
        ...
        getInactiveTabIndexes(viewBinding.mainViewPager.currentItem).forEach {
            viewBinding.mainTabsHolder.getTabAt(it)?.icon?.applyColorFilter(config.textColor)
        }

        val adjustedPrimaryColor = getAdjustedPrimaryColor()
        viewBinding.mainTabsHolder.background = ColorDrawable(config.backgroundColor)
        viewBinding.mainTabsHolder.setSelectedTabIndicatorColor(adjustedPrimaryColor)
        viewBinding.mainTabsHolder.getTabAt(viewBinding.mainViewPager.currentItem)?.icon?.applyColorFilter(adjustedPrimaryColor)

        if (viewBinding.mainViewPager.adapter == null && mWasProtectionHandled) {
            initFragments()
        }
    }
    ...
    override fun onPause() {
        super.onPause()
        storeStateVariables()
        config.lastUsedViewPagerPage = viewBinding.mainViewPager.currentItem
    }
    ...
    override fun onRestoreInstanceState(savedInstanceState: Bundle) {
        ...
        if (viewBinding.mainViewPager.adapter == null) {
    ...
    private fun initFragments() {
        viewBinding.mainViewPager.adapter = ViewPagerAdapter(this)
        viewBinding.mainViewPager.offscreenPageLimit = 2
        viewBinding.mainViewPager.currentItem = config.lastUsedViewPagerPage
        viewBinding.mainViewPager.onPageChangeListener {
            viewBinding.mainTabsHolder.getTabAt(it)?.select()
            invalidateOptionsMenu()
        }

        val tabToOpen = config.lastUsedViewPagerPage
        viewBinding.mainViewPager.currentItem = tabToOpen
        viewBinding.mainTabsHolder.onTabSelectionChanged(
            tabUnselectedAction = {
                it.icon?.applyColorFilter(config.textColor)
            },
            tabSelectedAction = {
                viewBinding.mainViewPager.currentItem = it.position
        ...
        viewBinding.mainViewPager.addOnPageChangeListener(object : ViewPager.OnPageChangeListener {
            ...
            override fun onPageSelected(position: Int) {
                viewBinding.mainTabsHolder.getTabAt(position)?.select()
                getAllFragments().forEach {
        ...
        viewBinding.mainTabsHolder.removeAllTabs()
        var skippedTabs = 0
        tabsList.forEachIndexed { index, value ->
            if (config.showTabs and value == 0) {
                skippedTabs++
            } else {
                val tab = viewBinding.mainTabsHolder.newTab().setIcon(getTabIcon(index))
                viewBinding.mainTabsHolder.addTab(tab, index - skippedTabs, config.lastUsedViewPagerPage == index - skippedTabs)
            }
        }

        // selecting the proper tab sometimes glitches, add an extra selector to make sure we have it right
        viewBinding.mainTabsHolder.onGlobalLayout {
            Handler().postDelayed({
                viewBinding.mainTabsHolder.getTabAt(config.lastUsedViewPagerPage)?.select()
                invalidateOptionsMenu()
            }, 100L)
        }

        viewBinding.mainTabsHolder.beVisibleIf(skippedTabs < tabsList.size - 1)
    }

    private fun setupTabColors(lastUsedTab: Int) {
        viewBinding.mainTabsHolder.apply {
    ...
    private fun updateTabColors() {
        ...
        viewBinding.mainTabsHolder.getTabAt(viewBinding.mainViewPager.currentItem)?.icon?.applyColorFilter(getAdjustedPrimaryColor())
    }
    ...
    private fun getCurrentFragment(): MyViewPagerFragment? {
        ...
        return fragments.getOrNull(viewBinding.mainViewPager.currentItem)
    }
