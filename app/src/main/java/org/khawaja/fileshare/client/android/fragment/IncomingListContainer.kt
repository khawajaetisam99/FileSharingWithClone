 ...
 */
class IncomingListContainer : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val adapter = TransferHistoryAdapterIncoming(gibSubscriberListener) {
            when (it) {
                is TransferHistoryAdapterIncoming.Click.Local -> {
                ...
                is TransferHistoryAdapterIncoming.Click.Text -> findNavController().navigate(
                    TransferHistoryFragmentDirections.actionTransferHistoryFragmentToNavTextEditor(
                        sharedText = it.text
                    )
                )
                is TransferHistoryAdapterIncoming.Click.Web -> findNavController().navigate(
        ...
        viewModel.transfersIncoming.observe(viewLifecycleOwner) {
            Log.d("submit_list_called:", "normal")
            adapter.submitList(it)
            emptyContentViewModel.with(recyclerView, it.isNotEmpty())
            //emptyContentViewModel.with(recyclerView, true)
        }
...
class TransferHistoryAdapterIncoming(
...
) : ListAdapter<ListItem, RecyclerView.ViewHolder>(TransfersItemCallbackIncoming()), Filterable {
...
class TransfersItemCallbackIncoming : DiffUtil.ItemCallback<ListItem>() {
