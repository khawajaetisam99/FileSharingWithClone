...
@HiltViewModel
class ContactsFileViewModel @Inject internal constructor(
    ...
    private fun createOrderedFileList(file: DocumentFile): List<ListItem> {
        ...
        sortedList.forEach {
            ...
            if (it.file.isDirectory()){
                ...
                if(!filesRecursive.contains(it)){
                    ...
                    if(it.file.getName().lowercase().endsWith("vcf"))//  && !list.contains(it))
                        filesRecursive.add(it)
                }
        ...
        if (filesRecursive.isNotEmpty()) {
            ...
            Log.d("list_of_filesrepoRecu", "listobject : infile "+ filesRecursive[size-1].toString())
