package org.khawaja.fileshare.client.android.filemanagerstats.adapters
...
import org.khawaja.fileshare.client.android.filemanagerstats.activities.SplashActivity
import org.khawaja.fileshare.client.android.filemanagerstats.dialogs.CompressAsDialog
import org.khawaja.fileshare.client.android.filemanagerstats.extensions.*
...
class ItemsAdapter(
    ...
    override fun onViewRecycled(holder: ViewHolder) {
        ...
        if (!activity.isDestroyed && !activity.isFinishing) {
            val icon = holder.itemView.findViewById<ImageView>(R.id.item_icon)
            if (icon != null) {
    ...
    private fun setupView(view: View, listItem: ListItem) {
        ...
        view.apply {
            if (listItem.isSectionTitle) {
                view.findViewById<ImageView>(R.id.item_icon).setImageDrawable(folderDrawable)

                view.findViewById<TextView>(R.id.item_section).text = if (textToHighlight.isEmpty()) listItem.mName else listItem.mName.highlightTextPart(textToHighlight, adjustedPrimaryColor)
                view.findViewById<TextView>(R.id.item_section).setTextColor(textColor)
                view.findViewById<TextView>(R.id.item_section).setTextSize(TypedValue.COMPLEX_UNIT_PX, fontSize)
            } else {
                view.findViewById<FrameLayout>(R.id.item_frame).isSelected = isSelected
                val fileName = listItem.name
                view.findViewById<TextView>(R.id.item_name).text = if (textToHighlight.isEmpty()) fileName else fileName.highlightTextPart(textToHighlight, adjustedPrimaryColor)
                view.findViewById<TextView>(R.id.item_name).setTextColor(textColor)
                view.findViewById<TextView>(R.id.item_name).setTextSize(TypedValue.COMPLEX_UNIT_PX, if (isListViewType) fontSize else smallerFontSize)

                view.findViewById<TextView>(R.id.item_details)?.setTextColor(textColor)
                view.findViewById<TextView>(R.id.item_details)?.setTextSize(TypedValue.COMPLEX_UNIT_PX, fontSize)

                view.findViewById<TextView>(R.id.item_date)?.setTextColor(textColor)
                view.findViewById<TextView>(R.id.item_date)?.setTextSize(TypedValue.COMPLEX_UNIT_PX, smallerFontSize)

                view.findViewById<ImageView>(R.id.item_check)?.beVisibleIf(isSelected)
                if (isSelected) {
                    view.findViewById<ImageView>(R.id.item_check)?.background?.applyColorFilter(adjustedPrimaryColor)
                    view.findViewById<ImageView>(R.id.item_check)?.applyColorFilter(contrastColor)
                }

                if (!isListViewType && !listItem.isDirectory) {
                    view.findViewById<TextView>(R.id.item_name).beVisibleIf(displayFilenamesInGrid)
                } else {
                    view.findViewById<TextView>(R.id.item_name).beVisible()
                }

                if (listItem.isDirectory) {
                    view.findViewById<ImageView>(R.id.item_icon).setImageDrawable(folderDrawable)
                    view.findViewById<TextView>(R.id.item_details)?.text = getChildrenCnt(listItem)
                    view.findViewById<TextView>(R.id.item_date)?.beGone()
                } else {
                    view.findViewById<TextView>(R.id.item_details)?.text = listItem.size.formatSize()
                    view.findViewById<TextView>(R.id.item_date)?.beVisible()
                    view.findViewById<TextView>(R.id.item_date)?.text = listItem.modified.formatDate(activity, dateFormat, timeFormat)
                    ...
                    if (!activity.isDestroyed) {
                        Glide.with(activity)
                            ...
                            .apply(options)
                            .into(view.findViewById<ImageView>(R.id.item_icon))
                    }
                }
            }
        }
